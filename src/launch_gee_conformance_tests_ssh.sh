...
launch_gee_tests_with_report_ssh() {
	...
	SSH "CURRENT_DATE_UTC=${CURRENT_DATE_UTC} GEE_TESTS=\"${GEE_TESTS}\" /tmp/conformance_tests/cukinia/cukinia -f junitxml -o /tmp/conformance_tests/GEISA-EE-tests/geisa-ee-conformance-report.xml /tmp/conformance_tests/GEISA-EE-tests/cukinia.conf"
	ee_test_exit_code=$?

	echo ""
	echo "Copying tests report on host"
	mkdir -p "${topdir}"/reports
	SCP "${board_user}@[${board_ip}]:/tmp/conformance_tests/GEISA-EE-tests/geisa-ee-conformance-report.xml" "${topdir}"/reports 1>/dev/null || {
		echo -e "${RED}Error:${ENDCOLOR} Failed to copy test report from board"
		exit 1
	}

	export ee_test_exit_code
}
...
launch_bandwidth_test_with_report_ssh() {
	...
	if [[ -z "${GEE_TESTS}" || " ${GEE_TESTS} " == *" connectivity_tests_bandwidth "* ]]; then
		...
		SSH "/tmp/conformance_tests/cukinia/cukinia -f junitxml -o /tmp/conformance_tests/GEISA-EE-tests/geisa-ee-conformance-report-bandwidth.xml /tmp/conformance_tests/GEISA-EE-tests/connectivity_tests_bandwidth.conf"
		bandwidth_test_exit_code=$?

		echo ""
		echo "Copying bandwidth test report on host"
		mkdir -p "${topdir}"/reports
		SCP "${board_user}@[${board_ip}]:/tmp/conformance_tests/GEISA-EE-tests/geisa-ee-conformance-report-bandwidth.xml" "${topdir}"/reports 1>/dev/null || {
